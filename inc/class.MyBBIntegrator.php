 ...
 *
 * So far the MyBBIntegrator includes following features:
 * - createCategory: Insert a new Category
 * - createForum: Insert a new Forum
 * - createPost: Insert a Post into Database
 * - createThread: Insert a new thread
 * - createUser: Insert a new User into Database
 * - dbEscape: Escape Values for Database usage
 * - generatePosthash: Generates a posthash
 * - getBusyThreadsWithinTimespan: Get hot topics within a defined timespan
 ...
 * - removeThread: Delete a thread from database
 * - removeUser: Delete a user from database
 * - updatePasswordOfUser: Change the Password of a User
 * - updateUser: Update the Data of a User
 *
...
{	
	...
	*/
	...
	/**
	 * Path of the forum, NO TRAILING SLASH
	 *
	 * @var array
	*/
	var $mybb_path = '../forums/MyBB_1.4.4';
	...
	function checkForumPassword($forum_id, $password = '', $pid = 0)
	{
		...
		if(!empty($parents))
		{
			foreach($parents as $parent_id)
			{
				...
				{
					if (!$this->checkForumPassword($parent_id, $forum_id))
					{
						return false;
					}
				}
	...
	*/
	function login($username, $password)
	{
		...
		*/
		 ...
		 * Has no effect yet, but could be good for a ToDo
		*/
